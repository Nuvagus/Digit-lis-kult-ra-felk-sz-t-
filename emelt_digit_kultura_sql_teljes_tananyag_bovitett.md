...
# 3. SQL syntax
Az SQL-lekérdezések mindig ugyanarra az alaplogikára épülnek: megadjuk, milyen adatokat szeretnénk látni (`SELECT`), melyik táblából (`FROM`), majd opcionálisan szűrünk, rendezünk vagy csoportosítunk. Az érettségin a helyes szintaxis alapfeltétel a pontszerzéshez.
...
# 4. CREATE DATABASE és DROP DATABASE
Az adatbázis létrehozása és törlése ritkábban szerepel közvetlenül az érettségin, de fontos megérteni, hogyan épül fel egy teljes adatbázisrendszer. Ezek a parancsok az adatbázis teljes szerkezetét kezelik.
...
# 5. CREATE TABLE és DROP TABLE
A táblák az adatbázis alapegységei. Minden adat táblákban tárolódik, ezért fontos tudni, hogyan hozhatunk létre megfelelő szerkezetet mezőkkel és kulcsokkal.
...
# 6. Adattípusok
Az adattípusok határozzák meg, hogy egy mező milyen adatot tárolhat. Az érettségin gyakori hiba a rossz adattípus választása, ezért fontos felismerni, mikor használunk számot, szöveget vagy dátumot.
...
# 7. PRIMARY KEY és FOREIGN KEY
Az elsődleges és idegen kulcsok biztosítják a táblák közötti kapcsolatokat. Ezek nélkül nem lehetne többtáblás adatbázist építeni vagy JOIN műveleteket használni.
...
# 8. INSERT INTO
Az `INSERT INTO` új rekordok beszúrására szolgál. Az érettségin gyakran kell új adatokat rögzíteni meglévő táblákba.
...
# 9. SELECT
A `SELECT` az SQL legfontosabb parancsa. Ezzel kérdezünk le adatokat az adatbázisból. Szinte minden érettségi feladat erre épül.
...
# 10. DISTINCT
A `DISTINCT` segítségével az ismétlődő értékeket szűrhetjük ki. Akkor használjuk, ha csak az egyedi értékek érdekelnek.
...
# 11. WHERE
A `WHERE` feltételes szűrésre szolgál. Segítségével csak azokat a rekordokat jelenítjük meg, amelyek megfelelnek a megadott feltételnek.
...
# 12. ORDER BY
Az `ORDER BY` a lekérdezések rendezésére szolgál. Az érettségin gyakori elvárás a névsorok vagy pontszámok rendezése.
...
# 13. AND / OR / NOT
A logikai operátorokkal több feltételt is kombinálhatunk. Ezek segítenek összetettebb szűrések elkészítésében.
...
# 14. UPDATE
Az `UPDATE` meglévő rekordok módosítására szolgál. Fontos, hogy mindig megfelelő `WHERE` feltételt használjunk, különben minden rekord módosulhat.
...
# 15. DELETE
A `DELETE` rekordok törlésére szolgál. Az érettségin gyakran kérnek feltételes törlést.
...
# 16. NULL értékek
A `NULL` nem üres szöveget jelent, hanem hiányzó adatot. A NULL kezelés az egyik leggyakoribb SQL-hibaforrás.
...
# 17. MIN és MAX
A `MIN` és `MAX` függvényekkel szélsőértékeket keresünk. Gyakran használjuk legjobb eredmény, legkorábbi dátum vagy legnagyobb érték meghatározására.
...
# 18. COUNT, SUM és AVG
Az aggregáló függvények összesítéseket készítenek. Az érettségin szinte minden adatbázis-feladat tartalmaz valamilyen statisztikai számítást.
...
# 19. LIKE és wildcardok
A `LIKE` szövegminták keresésére szolgál. Akkor használjuk, ha egy szöveg csak részben ismert.
...
# 20. IN
Az `IN` több lehetséges érték megadására szolgál. Rövidebbé és átláthatóbbá teszi a lekérdezéseket.
...
# 21. BETWEEN
A `BETWEEN` tartományos szűrésre használható. Gyakori példák a dátumtartományok és pontszám-intervallumok.
...
# 22. Aliasok
Az aliasok segítségével átnevezhetjük a táblákat vagy oszlopokat. Ez különösen hosszabb lekérdezéseknél teszi olvashatóbbá a kódot.
...
# 23. CONCAT és számított mezők
A `CONCAT` segítségével több szöveget kapcsolhatunk össze. Gyakran használjuk teljes nevek vagy összetett megjelenítések létrehozására.
...
# 24. Dátumkezelés
A dátumfüggvényekkel dátumokból nyerhetünk ki adatokat vagy számolhatunk időintervallumokat. Az érettségin rendszeresen előkerülnek.
...
# 25. CASE
A `CASE` feltételes logikát valósít meg SQL-ben. Hasonló az Excel HA függvényéhez.
...
# 26. JOIN
A `JOIN` több tábla összekapcsolására szolgál. Ez az emelt szintű adatbázis-feladatok egyik legfontosabb része.
...
# 27. LEFT JOIN
A `LEFT JOIN` akkor is megjeleníti a bal oldali tábla rekordjait, ha nincs hozzájuk kapcsolódó adat. Gyakran használjuk hiányzó kapcsolatok keresésére.
...
# 28. GROUP BY
A `GROUP BY` csoportosítja az adatokat. Ezzel készíthetünk városonkénti, eseményenkénti vagy sportágankénti összesítéseket.
...
# 29. HAVING
A `HAVING` a csoportosított eredmények szűrésére szolgál. Akkor használjuk, ha aggregáló függvény alapján szeretnénk szűrni.
...
# 30. Függvénykombinációk
Az érettségin ritkán elegendő egyetlen függvény használata. A legtöbb feladat több SQL-függvény kombinálását igényli.
...
# 31. Allekérdezések
Az allekérdezések egy másik lekérdezés eredményét használják fel. Ezek az emelt szint egyik legfontosabb témakörei.
...
# 32. EXISTS
Az `EXISTS` azt vizsgálja, hogy egy allekérdezés ad-e vissza találatot. Összetett szűrésekben gyakran használjuk.
...
# 33. CREATE TABLE AS
A `CREATE TABLE AS` segítségével egy lekérdezés eredményéből új táblát hozhatunk létre.
...
# 34. INSERT INTO SELECT
Az `INSERT INTO SELECT` lehetővé teszi, hogy egy lekérdezés eredményét másik táblába másoljuk.
...
# 35. Összetett érettségi feladatok
Az emelt szintű feladatok több SQL-eszköz együttes használatát kérik. Ilyenkor a JOIN, GROUP BY, HAVING és allekérdezések kombinációja jelenik meg.
...
# 36. Tipikus érettségi hibák
Az adatbázis-feladatoknál sok pontvesztés apró szintaktikai vagy logikai hibák miatt történik. Ezek ismerete segít stabilan magas pontszámot szerezni.
